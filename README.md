...
- **PPT production tool**: `run_ppt_production` for durable PPT P0/P1 flows with outline review, editable native `.pptx` generation, slide previews, quality report views, input recording, revision impact analysis, targeted outline/deck-slide revisions, page-level stale preview state, per-slide PPTX segments, stale slide preview/segment regeneration, and final artifact tracking.
