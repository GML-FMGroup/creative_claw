...
如果只想运行短视频能力，请先看 [short_video_production_zh.md](short_video_production_zh.md)。如果要运行 PPT P0 能力，请看 [ppt_production_zh.md](ppt_production_zh.md)。如果要看当前短视频生成实现细节，请看 [short_video_generation_framework_zh.md](short_video_generation_framework_zh.md)。
...
- 调用业务线 typed tool，例如 `run_short_video_production(...)` 或 `run_ppt_production(...)`。
...
PPT 或后续短剧剧本不应复用短视频私有 schema，而应复用共享 production 基础设施，再定义自己的业务 state、review payload、quality check 和 projection。当前 PPT P0 已采用 `src/production/ppt/` 独立业务线结构。
