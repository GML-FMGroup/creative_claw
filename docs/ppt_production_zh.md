...
当前目标是在 P0/P1 闭环上推进 targeted revision 和页面级中间产物重生成：用户给出 brief，系统默认通过 PPT 私有 ADK structured expert 生成可审阅 outline；如果 expert 不可用或输出不合格，会回退到确定式 fallback，并把原因写入 warning / production event。如果启动时传 `render_settings.brief_review=true`，系统会先暂停到 `brief_review`，让用户确认方向后再生成 outline。用户确认 outline 后生成可审阅 `DeckSpec`；用户确认 deck spec 后生成可编辑 PPTX、每页 preview、每页单独 PPTX segment 和质量报告；用户再确认 preview 后完成并投影最终产物到 ADK session state。若用户附加 TXT/MD/DOCX/PDF 源文档、PPTX 模板或参考图，系统会先生成轻量 `DocumentSummary` / `TemplateSummary`，并把可用上下文纳入 brief review、outline 和 deck spec。用户后续明确修改某一页或某个 outline item 时，系统会优先做目标级 stale 标记；对 deck slide 级修改，可以先只重建 stale 页面的 preview 和单页 segment，再等用户确认后重建完整 final PPTX。
...
- outline 和 deck spec 默认通过 PPT 私有 ADK structured expert 生成；失败时回退到确定式 fallback。
...
- `final_preview_review`：生成后审阅 preview PNG、每页可编辑 PPTX segment、slide 级 source refs 和质量报告；review payload 的 `metadata` 会附带 final/preview/segment 路径摘要、preview renderer/fallback 摘要、quality status、check 计数和 warning/fail check 摘要。
- `status` / `view` 查询当前状态、中间结果、事件和产物。
- `add_inputs` 追加 PPT 模板、源文档和参考图，并回到 outline review。
- TXT/MD/DOCX/PDF 源文档轻量抽取，生成 `DocumentSummary`，把关键事实提供给 PPT planning，并把源文档 ref 透传到对应 deck slide；PDF 仅支持可抽取文本层，不包含 OCR 或复杂版面理解。
- PPTX 模板轻量分析，生成 `TemplateSummary`，包括 slide/layout/master/theme/media 等结构信号。
- 参考图作为轻量 visual/brand context 进入 PPT planning；当前不做图像理解，不会从图片内容推断未描述事实。
...
- reference image / logo / brand asset 的 slide-level asset plan 和图像理解。
...
`state.json` 是事实源；Markdown/JSON 视图、preview 和 render manifest 都是投影，可由 state 重建。`view_type="manifest"` 会返回同一份结构化清单，适合 App 或 CLI 一次性读取交付路径。`deck_spec`、`deck_spec.md`、preview review payload、`view_type="previews"` 和 render manifest 会保留 slide 级 `source_refs`，并在投影里提供 `source_ref_details`（源文件名、路径、输入角色和状态），用于轻量追踪页面来自哪些源文档；这不是精确页码/段落引用。最终预览 review payload 还会在 `metadata.delivery`、`metadata.preview` 和 `metadata.quality` 中暴露交付路径摘要、preview renderer 计数、fallback 原因、质量状态、check 计数和需要关注的 warning/fail check，方便前端或 orchestrator 在用户审批前直接展示风险信号。质量报告会做确定性结构/内容/交付检查；`quality_report.md` 会列出每个 check 的状态、摘要和关键 details。PPTX slide count inspect 会保留 `inspection_method` 和 `inspection_error`，避免把损坏文件、依赖失败和无 slide XML 都混成 `actual=0`。当源文档成功抽取出 `salient_facts` 时，还会检查至少有一个源事实进入 outline 或 deck spec，并在 `source_fact_coverage` details 里列出源文件 details、matched fact 样例和 unmatched fact 样例，未覆盖时给 warning，便于人工复核事实遗漏风险。
...
Preview 优先使用 LibreOffice + Poppler 渲染真实页面图。如果缺失或渲染失败，会降级到 Pillow 生成确定性 preview PNG。降级原因会进入 `SlidePreview.metadata.fallback_reason`、`final_preview_review.metadata.preview`、`state.warnings` 和 `ppt_preview_renderer_fallback` production event。

## P1 验收清单

PPT 单元测试覆盖 manager、preview、quality、source refs、targeted revision 和 eval asset schema。P1 闭环验收应覆盖以下路径：带源文档启动 PPT production；确认 structured expert 或 fallback 基于 source facts 规划 outline；approve 后进入 `deck_spec_review` 并保留 slide 级 `source_refs` / `source_ref_details`；approve 后生成 final PPTX、preview PNG、单页 segment、manifest 和 quality report；`final_preview_review.metadata` 暴露 delivery、preview renderer 和 quality 摘要；最终 approve 后状态进入 `completed`，并把 `final.pptx`、`quality_report.md` 和 `render_manifest.md` 投影到 ADK session state，同时不把中间 segment 当作最终交付文件。

ADK eval 资产位于 `tests/eval/evalsets/ppt_p0_evalset.json`，覆盖 PPT 正向路由、源文档输入、brief review、view、revision impact 和 confirmed revision。真实 eval 需要模型凭据和网络；单元测试只校验 evalset/config schema。
