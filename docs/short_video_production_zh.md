...
本文档记录当前短视频生产 P0 的可用范围、运行方式和验收检查项。P0 重点是先形成一个可控的单镜头生产闭环：用户给出任务描述，系统先返回资产计划让用户确认，用户确认后再调用视频模型生成最终 MP4。
...
1. 系统返回 `asset_plan_review`，展示视频类型、比例、Seedance 模型、参考素材、镜头 prompt 和音频计划。
2. 用户确认，例如回复 `确认，9:16`。
3. 系统调用 Seedance 生成原生有声视频。
4. 系统渲染、校验并返回最终 MP4 路径。
