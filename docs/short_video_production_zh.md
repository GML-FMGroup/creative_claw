...
本文档记录当前短视频生产 P1a 的可用范围、运行方式和验收检查项。P1a 在 P0 单镜头真实生成闭环前增加了 storyboard 审阅：用户给出任务描述后，系统先返回多镜头 storyboard 让用户确认；确认 storyboard 后再返回 provider 资产计划；用户再次确认后才调用视频模型生成最终 MP4。
...
P1a 已做 storyboard 审阅，但暂不做真实多镜头 provider 分段生成、rough cut 二次确认、局部镜头重生成和自动质量评估。这些属于 P1b/P1c/P1e。
...
5. 系统调用 Seedance 生成原生有声视频。
6. 系统渲染、校验并返回最终 MP4 路径。
...
- 确认后，进度卡出现生成视频、准备音频、渲染、校验、完成等阶段。
- 最终回复包含 MP4 产物路径。
