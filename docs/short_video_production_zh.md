...
本文档记录当前短视频生产 P1d 的可用范围、运行方式和验收检查项。P1d 在 P0 真实生成闭环、P1a storyboard 审阅、P1b 分段确认、P1c 局部重生成基础上，增加了显式 provider/runtime 选择：用户给出任务描述后，系统先返回 storyboard，再返回 provider 资产计划；用户确认资产计划后，系统生成可预览的短视频分段，等待用户确认后再继续生成后续分段或合成最终 MP4。用户修改某个分段时，系统只让受影响分段过期，保留其他已确认分段。
...
当前 P1d 已做真实 provider 分段生成、`shot_review` 暂停、当前分段修改、显式 `shot_asset_plan` / `shot_artifact` 目标的局部过期和局部重生成，以及 Seedance / Seedance fast / Veo+TTS 的显式选择。仍未做自动质量评分、跨分段视觉一致性增强和完整 rough cut 编辑台，这些属于后续 P1e 或质量迭代优化。
