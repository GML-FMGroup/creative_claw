"""Per-request route context for realtime step publishing."""
...
def get_route() -> tuple[str | None, str | None]:
    """Return the active channel route for the current request context."""
    return _ROUTE_CHANNEL.get(), _ROUTE_CHAT_ID.get()


@contextmanager
def route_context(channel: str, chat_id: str) -> Iterator[None]:
    """Temporarily bind one channel route for nested async tool callbacks."""
