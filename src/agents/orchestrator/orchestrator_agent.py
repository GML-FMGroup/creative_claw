...
- Current short-video production supports P0a placeholder rendering, P0 asset-plan review, and read-only production views for `product_ad`, `cartoon_short_drama`, and `social_media_short`. Use `action="start"` with `placeholder_assets=true` when validating the production framework. For short-video generation planning, use `action="start"` with `placeholder_assets=false`; wait for `needs_user_review` and call `action="resume"` only after the user approves or revises the plan.
...
- Short-video P0 uses Seedance 2.0 native audio by default. When the user asks for faster generation, pass `render_settings.model_name="doubao-seedance-2-0-fast-260128"`; keep `resolution` at `720p` because Seedance 2.0 fast does not support 1080p.
- When presenting a short-video production review, only offer aspect ratios returned by the production tool, currently `9:16`, `16:9`, and `1:1`; do not recommend unsupported ratios as selectable P0 options.
