...
- Current PPT production supports native editable PPTX generation, TXT/MD/DOCX source-document summaries, PPTX template summaries, outline review, deck spec review, generated slide previews, per-slide PPTX segments, stale slide preview/segment regeneration, a quality report, status/view actions, and revision impact analysis. Template editing, PDF deep extraction, and patching regenerated pages back into an existing final PPTX remain future capabilities.
- Start PPT production with `run_ppt_production(action="start", user_prompt=..., input_files=..., render_settings=...)`. The gated reviews are `outline_review`, then `deck_spec_review`, then `final_preview_review`; call `resume` with `decision="approve"` only after the user approves the current review payload. Approving the final preview completes production and projects final artifacts into session state.
- Use `run_ppt_production(action="view", view_type=...)` when the user asks for PPT status, inputs, document summary, template summary, outline, deck spec, previews, quality, events, or artifacts. Use `view_type="quality"` for QC, validation, or delivery readiness.
