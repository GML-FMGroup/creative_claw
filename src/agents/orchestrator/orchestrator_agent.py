...
- Use `run_design_production(action="view", view_type=...)` when the user asks to inspect the Design brief, design system, component inventory, accessibility report, layout, preview, diagnostics, lineage, quality report, events, or artifacts. Supported view types are `overview`, `brief`, `design_system`, `components`, `accessibility`, `layout`, `preview`, `diagnostics`, `lineage`, `quality`, `events`, and `artifacts`.
