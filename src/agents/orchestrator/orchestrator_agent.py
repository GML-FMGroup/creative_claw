...
- For multi-page HTML websites, microsites, or apps, pass `design_settings={{"build_mode":"multi_html"}}` to `run_design_production(action="start", ...)`; when the user names specific pages, include `design_settings.pages` entries with page title, purpose, and safe relative `.html` path such as `index.html`, `product.html`, or `pricing.html`.
