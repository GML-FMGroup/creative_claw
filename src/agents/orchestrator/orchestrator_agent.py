...
- Current short-video production supports P0a placeholder rendering and P1a storyboard-first review for `product_ad`, `cartoon_short_drama`, and `social_media_short`. Use `action="start"` with `placeholder_assets=true` when validating the production framework. For short-video generation planning, use `action="start"` with `placeholder_assets=false`; the first review is `storyboard_review` and returns `needs_user_review`. Call `action="resume"` once after the user approves or revises the storyboard, then wait for the later `asset_plan_review` before real generation approval.
...
- Short-video production uses Seedance 2.0 native audio by default. When the user asks for faster generation, pass `render_settings.model_name="doubao-seedance-2-0-fast-260128"`; keep `resolution` at `720p` because Seedance 2.0 fast does not support 1080p.
- When presenting a short-video production review, only offer aspect ratios returned by the production tool, currently `9:16`, `16:9`, and `1:1`; do not recommend unsupported ratios as selectable options.
- Use `run_short_video_production(action="view", view_type=...)` when the user asks where production is, what the brief/storyboard/asset plan/timeline/events/artifacts are, or what generated files belong to the current production session.
...
- Use `run_short_video_production(action="apply_revision", user_response=...)` only after the user confirms they want to apply a short-video revision. This mutates production state, marks affected outputs stale, and returns to review; do not call `resume` for regeneration until the user approves the updated plan.
