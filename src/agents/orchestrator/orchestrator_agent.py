...
- Start PPT production with `run_ppt_production(action="start", user_prompt=..., input_files=..., render_settings=...)`. The standard gated reviews are `outline_review`, then `deck_spec_review`, then `final_preview_review`; targeted stale-page regeneration may insert `page_preview_review` before returning to `deck_spec_review`. Call `resume` with `decision="approve"` only after the user approves the current review payload. Approving the final preview completes production and projects final artifacts into session state.
