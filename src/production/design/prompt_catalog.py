"""Prompt-template loading for Design production."""
...
class DesignPromptCatalogError(ValueError):
    """Raised when a Design prompt template cannot be rendered safely."""


_PACKAGE = "src.production.design.prompts"
_TEMPLATE_NAME_RE = re.compile(r"^[a-z0-9_]+$")
_PLACEHOLDER_RE = re.compile(r"{{\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*}}")


def available_prompt_templates() -> list[str]:
    """Return packaged Design prompt template names."""
...
def render_prompt_template(template_name: str, variables: Mapping[str, Any]) -> str:
    """Render a packaged Design prompt template with strict variable validation."""
    ...
    if missing:
        raise DesignPromptCatalogError(
...
def _load_prompt_template(template_name: str) -> str:
    ...
    if not _TEMPLATE_NAME_RE.fullmatch(normalized):
        raise DesignPromptCatalogError(f"Invalid prompt template name: {template_name!r}")
    prompt_path = resources.files(_PACKAGE).joinpath(f"{normalized}.md")
    if not prompt_path.is_file():
        raise DesignPromptCatalogError(f"Unknown prompt template: {normalized}")
