...
- **PPT 生产工具**：`run_ppt_production`，用于 PPT P0/P1 的持久化生产流程，支持可选 brief 审阅、outline 审阅、页面 preview 审阅、原生可编辑 `.pptx` 生成、页面 preview、render manifest 读取、质量报告读取、输入记录、修改影响分析、目标级 outline/deck slide 修订、页面级 preview stale 状态、单页 PPTX segment、stale 页面 preview/segment 重生成和最终产物追踪。
...
- [docs/ppt_production_zh.md](docs/ppt_production_zh.md)：PPT 的可选 brief 审阅、outline/deck spec 审阅、原生可编辑 PPTX 生成、preview、render manifest、质量报告、目标级修订、降级策略和 P1 边界
