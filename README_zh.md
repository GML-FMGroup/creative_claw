...
- **PPT 生产工具**：`run_ppt_production`，用于 PPT P0 的持久化生产流程，支持 outline 审阅、原生可编辑 `.pptx` 生成、页面 preview、质量报告读取、输入记录、修改影响分析、目标级 outline/deck slide 修订、页面级 preview stale 状态、单页 PPTX segment 和最终产物追踪。
