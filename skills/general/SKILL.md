...
5. If shell execution is clearly the fastest safe path, use `exec`.
6. If the task needs external information, use `web_search` and `web_fetch`.
7. If the task is already complete, call `finish_task`.
